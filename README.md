...
- ✅ `#水群查询` 统计维度升级为：`时 / 日 / 周 / 月 / 年 / 总`
- ✅ 个人统计页面（`resources/user-stats`）重写：周期统计与消息类型统计统一为六维展示
- ✅ AI 提示词模板开放为用户可配置项：支持在 `data/config/group-analysis.json` 自定义
- ✅ Guoba 配置页提示词输入改为多行文本框，便于直接编辑长提示词
