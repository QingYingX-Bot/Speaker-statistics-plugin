...
class DataService {
    constructor() {
        ...
        // 当前群列表缓存（运行时群列表暂不可用时作为兜底，与 #群列表 一致）
     ...
     * 归档群（cleanZombieGroups 归档的）不计入统计且不可查；运行时群列表缺失时回退缓存
     * @returns {string[]|null} 当前群 ID 数组，null 表示无法获取
     */
    ...
    /**
     * 从 Bot.gl 刷新当前群列表缓存（供运行时群列表缺失时回退）
    ...
    async getGlobalStats(page = 1, pageSize = 9) {
        try {
            ...
            // 仅统计 Bot.gl 中的群（与 RobotManagement-plugin #群列表/#群员统计 一致）；归档群不计入；优先 Bot.gl，缺失时回退缓存
