...
- 新增 `#上月水群榜` 命令，可查看当前群上月发言排行。
- 排行榜合并 QQBot OpenID 与已绑定 QQ 号数据，个人卡片和背景展示同步兼容合并 ID。

### 🤖 水群分析

- AI 话题、金句和用户称号头像兼容 QQBot OpenID。
- 分析上下文保留群 ID，渲染报告时可生成正确头像地址。

### 🌐 Web 管理端

- 新增 `web.ipBlock` 配置和 Guoba 配置项，支持重复拒绝访问 IP 临时封禁。

### 🔧 稳定性

- 修复艾特记录时间异常时可能写入无效 Redis 记录的问题。
- 用户最近消息查询改为字符串 ID 匹配，兼容 OpenID 和 `self_id:openid`。
- `index.js` 兜底版本更新为 `5.1.2`，并同步 `package.json` / `package-lock.json` 版本。
...
- 重新提供轻量 Web 管理端，支持统计概览、群组列表和排行查询。
- 新增 `data/config/web.json` 配置分组，并接入 Guoba 配置面板。
- 新增 `web.queryLog` 开关，控制网页查询概览和群组列表时的全局统计计算日志。
- 修复网页排行在未填写群号时日榜、周榜、月榜、年榜无法查看的问题，改为按全部当前群聚合统计。

### 🔁 Redis 统计同步

- 新增主人命令 `#水群同步Redis统计`，可从数据库补当前群 Redis 消息类型统计。
- 新增主人命令 `#水群总同步Redis统计`，可从数据库补全部群 Redis 消息类型统计。
- 同步写入使用 `database_sync` 来源标记，刷新时会替换旧同步占位，并保护已有真实采集 Key。

### 🪪 昵称展示

- `#水群查询`、`#水群查询群列表` 与排行榜展示昵称时优先读取当前群成员名片 / 群昵称。
- 总榜和周期榜会在渲染前按当前群上下文覆盖展示昵称，统计聚合仍保持用户 ID 口径。

### 🎨 排行榜背景

- 新增私聊命令 `#水群设置背景`，生成绑定当前用户的短期 Web 设置链接。
- 新增排行榜背景编辑器，支持上传图片、预览排行行效果、调整缩放与位置、保存和删除背景。
- 背景编辑器预览改为读取当前用户的总榜真实数据。
- 新增 `#水群背景设置` 作为背景设置命令别名。
- 新增背景 token 与背景文件服务，背景图片保存到 `data/backgrounds/ranking/`，并由排行榜渲染自动读取。
- 新增 Web 管理端“背景”页，支持管理所有用户的排行榜个人背景。
- Web 管理端 favicon 改为使用 `iconoir:bubble-search-solid` 图标。
- 新增 `web.backgroundEditor` 配置项，支持控制编辑器开关、链接有效期和图片大小限制。
- 新增 `web.allowExternalManageAccess` 配置项，可关闭公网普通管理端访问，同时保留局域网管理端和带 token 的背景设置入口。
- 新增 `web.accessLog` 配置项，记录页面访问、背景上传、删除和访问拒绝日志。
- 背景设置 token 缩短为 12 位短链接参数。

### ⚙️ 配置与发布

- `package.json` 与 `package-lock.json` 版本更新为 `5.1.1`。
- `README.md` 文档版本与发布日期更新为 `5.1.1 / 2026-06-07`。
- `.gitignore` 新增 `pnpm-lock.yaml` 忽略规则，继续以 `package-lock.json` 作为提交锁文件。
- `.gitignore` 新增 Web 源码目录忽略规则，Web 前端源码拆分到独立仓库，插件仓库仅提交 `resources/server` 构建产物。

## [5.0.2] - 2026-04-04 ~ 2026-04-05

### 🎨 渲染重写

- `#水群查询` 个人统计渲染模板重写，统计维度由原 `日 / 月 / 总` 升级为 `时 / 日 / 周 / 月 / 年 / 总`。
- 用户统计页面（`resources/user-stats`）改为六维表格化展示：
  - 周期统计：消息数、字数统一按六维展示。
  - 消息类型统计：文本、回复、艾特、表情、图片、链接、视频、媒体消息统一按六维展示。

### 📊 统计口径增强

- 用户查询命令（`#水群查询`）补充六维周期统计数据输出（时/日/周/月/年/总）。
- 文本回退模式同步升级为六维展示口径，图片渲染失败时也能看到完整周期统计。
- 消息类型统计口径同步升级为六维，并保留“基于消息采集数据”的说明。

### 🤖 水群分析增强

- `#水群分析` 新增日期参数 `昨日/昨天`（默认不带参数仍查询今日）。
- 报告冷却记录新增“报告日期”字段，查询时会区分今日/昨日，避免日期不一致时误判冷却。
- 相关日志与提示文案改为动态日期显示，便于排查“查询昨日命中今日冷却”的场景。

### ⚙️ 配置与可维护性

- AI 分析的三类提示词（话题分析 / 金句提取 / 用户称号）统一迁移为配置项：
  - `analysis.topic.promptTemplate`
  - `analysis.goldenQuote.promptTemplate`
  - `analysis.userTitle.promptTemplate`
- 默认提示词已写入 `config/templates/groupAnalysisTemplate.js` 与 `data/config/group-analysis.json`，支持用户直接修改。
- Guoba 配置（`guoba/schemas/groupAnalysis.js`）将上述提示词输入组件改为 `InputTextArea` 多行文本框，提升可编辑性。

### 📝 文档更新

- `README.md` 版本与发布日期更新为 `5.0.2 / 2026-04-04`。
- README 补充本次版本更新摘要，并同步标注 `#水群查询` 六维统计口径。
...
- 新增 `#水友分析 [at]` 命令：群内可直接分析指定成员当前群发言画像；未携带 `at` 时默认分析自己。
