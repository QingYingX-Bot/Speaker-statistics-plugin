...
/**
 * 根据条件类型添加额外字段（从配置格式转换为表单格式）
 * @param {Object} achievementData 表单格式的成就对象
 * @param {Object} condition 配置格式的条件对象
 */
function addConditionFields(achievementData, condition) {
...
export async function getConfigData() {
  try {
    ...
    const customAchievements = Object.entries(userAchievements).map(([id, achievement]) => {
      ...
      // 根据条件类型添加额外字段
      addConditionFields(achievementData, condition)
    ...
    try {
      ...
      globalConfig.error('[发言统计] 获取群专属成就失败:', err)
    }
